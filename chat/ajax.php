...
function Send()
{
	...
	$text = substr($_POST['text'], 0, 500); // обрезаем до 500 символов
if ($_SESSION['status'] != 2 && $_SESSION['status'] != 1) $text = htmlspecialchars($text); // заменяем опасные теги (<h1>,<br>, и прочие) на безопасные
...
function Load()
{
	...
	$query = mysql_query("SELECT * FROM `messages` WHERE ( `id` > $last_message_id ) ORDER BY `id` DESC LIMIT 1");
	...
	if( mysql_num_rows($query) > 0 )
	{
		...
		foreach ( $messages as $value )
		{
			...
			$js .= 'chat.append("<span>' . $value['name'] . '(' . $value['date'] . ')' . '&raquo; ' . $value['text'] . '</span>");'; // добавить сообщние (<span>Имя &raquo; текст сообщения</span>) в наш div 
		}
